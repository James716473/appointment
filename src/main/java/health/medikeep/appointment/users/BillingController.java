...
public class BillingController {
    ...
    @PutMapping("/")
    public ResponseEntity<?> payBill(@RequestBody BillingInfo billing) {
        if (billingRepository.payBill(billing.billing_id())) {
            return ResponseEntity.ok("Billing Paid Successfully");
        } else {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Billing Payment Unsuccessful");
        }
    }

    @PutMapping("/undo")
    public ResponseEntity<?> undoPayment(@RequestBody BillingInfo billing) {
        if (billingRepository.undoPayBill(billing.billing_id())) {
            return ResponseEntity.ok("Billing Payment Undone Successfully");
        } else {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Billing Payment Undo Unsuccessful");
        }
    }
